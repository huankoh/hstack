...
- Apply pstack's `hstack-principle-separate-before-serializing-shared-state` to source coordinates.
- Apply pstack's `hstack-principle-minimize-reader-load` and `hstack-unslop` skills to the final verdict.
...
Do a bounded source and history pass before choosing an owner or destination. Use pstack's `hstack-how` skill to trace the path from the reported action to the observed result. Use `hstack-why` when the report looks like a regression or touches defensive code.
