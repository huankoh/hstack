---
name: hstack-blast-radius
description: Within an explicitly selected hstack workflow. Find what a change could
  break somewhere else before it ships, beyond the diff, and prove the one fact it's
  safe because of by running real code instead of writing it up. Use for 'blast radius
...
Companion to `hstack-how` and `hstack-why`. `hstack-how` tells you what the code does. `hstack-why` tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use `hstack-why` step 2 to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Same rules as `hstack-why`. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, run it as an `hstack-arena`. Ask several models the same question and merge the answers. Different models catch different real bugs.
...
Write it through `hstack-unslop`, cite real code, and strip anything private before it goes anywhere public.
