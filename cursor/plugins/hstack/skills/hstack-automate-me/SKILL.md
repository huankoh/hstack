---
name: hstack-automate-me
description: Within an explicitly selected hstack workflow. Use for "automate me",
  "create/update/refresh my -mode skill", "turn/capture my preferences or working
  style into a skill", or wanting agents to follow how the user works. Drafts or revises
  a personal -mode skill via create-skill + unslop, optionally pulling fresh evidence
...
This skill orchestrates three others: an inline mining pass (see step 1), Cursor's built-in `create-skill` (authoring), and the **hstack-unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
The **hstack-poteto-mode** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as poteto-mode's.
...
Apply the **hstack-unslop** skill and `create-skill`'s writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- The **hstack-poteto-mode** skill: example of the output shape.
- The **hstack-unslop** skill: prose discipline for every line.
