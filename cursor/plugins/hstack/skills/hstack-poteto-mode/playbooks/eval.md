...
- No `eval`, `test`, `judge`, `experiment`, `rubric`, `score`, `compare`, `benchmark`, `candidate`, or `hstack-arena` in any directory, file, or prompt the candidate sees.
...
4. **Spawn N parallel candidates** on different models per the **hstack-arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different model family per the **hstack-arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
