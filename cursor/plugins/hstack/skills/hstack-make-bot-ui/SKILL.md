---
name: hstack-make-bot-ui
description: Within an explicitly selected hstack workflow. Use when building a custom
  UI (page, dashboard, buttons) that should wake a Grok Bot over a webhook, when the
  user must provide a webhook sender key, or when exposing that UI on Tailscale.
