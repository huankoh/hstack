...
- Use pstack's `hstack-principle-guard-the-context-window` for delegated analysis.
- Apply pstack's `hstack-principle-sequence-verifiable-units`, `hstack-principle-fix-root-causes`, and `hstack-principle-prove-it-works` through repro, fix, and verification.
...
Use pstack's `hstack-how` skill to trace the action through the repository. Use `hstack-why` for regression history and defensive code. Form competing cause hypotheses and identify evidence that would separate them.
...
- Invoke pstack's `hstack-tdd` skill when there is a cheap local test target, and write the failing test before the fix.
...
- Run the pull request text and all Slack updates through pstack's `hstack-unslop` skill.
