...
![She tastes a finished dish while robots cook from a recipe box, with pinned cards reading /hstack-how, /hstack-tdd, and /loop above the counter.](images/recipes.jpg)

## Understand an unfamiliar subsystem

```text
use /hstack-how first to understand how this initialization works. then use /hstack-why to figure out why it broke recently.
```
...
```text
ask /hstack-arena for a second opinion on this thread and our approach
```
...
```text
/hstack-interrogate the whole branch, but skeptically. don't change anything yet. no nitpicks unless it's an actual bug or regression in behavior.
```
...
```text
/hstack-poteto-mode repro the duplicate write first. if there's a cheap test path, /hstack-tdd it. then fix and rerun.
```
...
```text
/hstack-unslop that, no emdashes
```
...
That's the whole prompt. [`/hstack-bro`](../../skills/hstack-bro/SKILL.md) restates the last message like one human talking to another, no jargon, shorter. Use it when a reply is technically thorough and you still don't know what it said.

## The pitfalls

- **Enumerating skills in the prompt.** "use /hstack-how then /hstack-architect then /hstack-arena" reorders steps the playbook already sequences. State the goal and constraints. Name a skill only to override a default.
...
- **Using `/hstack-arena` for coverage.** `/hstack-arena` repeats one design or code brief, then picks a base and grafts the best parts. `/hstack-swarm` partitions slices or declared race arms and aggregates one report.
- **Accepting every review comment.** Bots and humans both file real catches and noise in one list. `/hstack-interrogate` sorts findings into act-on and dismissed buckets with reasons, and you can override either way.
...
- **Writing a `SKILL.md` freehand.** Route it through the [Authoring or modifying a skill playbook](../../skills/hstack-poteto-mode/playbooks/authoring-a-skill.md) so validation and review happen.
